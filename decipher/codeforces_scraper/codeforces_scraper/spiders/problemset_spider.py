...
class CodeforcesProblemSpider(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        if response.selector.xpath('//span[@class="inactive"]/text()').extract():
            if (
                response.selector.xpath('//span[@class="inactive"]/text()')[0].extract()
                != u"\u2192"
            ):
                next_page_href = response.selector.xpath(
                    '//div[@class="pagination"]/ul/li/a[@class="arrow"]/@href'
                )[0]
    ...
    def parse_problem_contents(self, response):
        ...
        item["p_title"] = response.selector.xpath(
            '//div[@class="header"]/div[@class="title"]/text()'
        )[0].extract()
        item["p_time_limit"] = response.selector.xpath(
            '//div[@class="header"]/div[@class="time-limit"]/text()'
        )[0].extract()
        item["p_memory_limit"] = response.selector.xpath(
            '//div[@class="header"]/div[@class="memory-limit"]/text()'
        )[0].extract()
        item["p_input_file"] = response.selector.xpath(
            '//div[@class="header"]/div[@class="input-file"]/text()'
        )[0].extract()
        item["p_output_file"] = response.selector.xpath(
            '//div[@class="header"]/div[@class="output-file"]/text()'
        )[0].extract()
        ...
        item["p_sample_tests"] = response.selector.xpath(
            '//div[@class="sample-tests"]/div[@class="sample-test"]/div'
        ).extract()
